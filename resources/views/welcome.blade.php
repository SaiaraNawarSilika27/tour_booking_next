                ...
                @foreach ($packages as $package)
                   <a href="{{ route('packages.view', $package->id) }}">
                        <figure class="relative w-64 h-80 ">
                            ...
                            @endif
                            <div class="absolute bottom-0 w-full p-4 bg-black bg-opacity-0 text-white rounded-b-lg">
                                <h2 class="card-title text-base">Title: {{ $package->name }}</h2>
                                <p class="text-sm">Price: ${{ $package->price }}</p>
                                <p class="text-sm">Duration: {{ $package->Duration ?? 'N/A' }}</p>
                                <p class="text-sm">Description: {{ Str::limit($package->description, 100) }}</p>
                            </div>
                        </figure>
                    </a>
                @endforeach
